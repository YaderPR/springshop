...
import { Loader2, AlertTriangle, Users, ShoppingBag, DollarSign, CheckCircle, CircleAlert } from 'lucide-react';
...
const StatCard = ({ title, value, icon, colorClass }: { title: string, value: string | number, icon: React.ReactNode, colorClass: string }) => (
  <div className={`bg-primary border ${colorClass} rounded-lg shadow-md p-6 flex items-center gap-4`}>
...
export default function AdminDashboard() {
  ...
  // Estado para nuestras estadísticas
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      ...
      setError(null);
      try {
        // 1. Llamamos a ambas APIs en paralelo para más eficiencia
        const [ordersData, usersData] = await Promise.all([
          getOrders(keycloak.token!),
          getUsers(keycloak.token!)
        ]);

        // 2. Calculamos las estadísticas de Órdenes
        const sales = ordersData.reduce((sum, order) => 
          order.status === 'PAID' ? sum + order.totalAmount : sum, 0
        );
        setTotalSales(sales);
        setTotalOrders(ordersData.length);
        
        // 3. Ordenamos por fecha y tomamos las 5 más recientes
        const sortedOrders = [...ordersData].sort((a, b) => 
          new Date(b.createAt).getTime() - new Date(a.createAt).getTime()
        );
        setRecentOrders(sortedOrders.slice(0, 5));

        // 4. Calculamos las estadísticas de Usuarios
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-2xl font-bold text-white mb-4">Órdenes Recientes</h2>
        <div className="bg-primary border border-gray-700 rounded-lg shadow-md">
          <div className="space-y-0">
            {recentOrders.length > 0 ? (
              recentOrders.map((order, index) => (
                ...
                >
                  <div className="space-y-1">
                    <span className="text-lg font-bold text-white">Orden #{order.id}</span>
              ...
              <p className="text-center p-8 text-gray-500">No se encontraron órdenes recientes.</p>
            )}
          </div>
          {totalOrders > 5 && (
            <Link 
