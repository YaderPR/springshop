...
class OrderApiService implements OrderService {
  ...
  Future<Map<String, dynamic>> processCheckout({
    ...
    print(
      '🛒 [OrderService] Preparando datos para checkout con redirectUrl: $redirectUrl',
    );
  ...
  @override
  Future<List<OrderResponseDto>> fetchOrdersByUserId(int userId) async {
    print('🔍 [OrderService] Solicitando órdenes para UserID: $userId');

    try {
      // Delegar la llamada al repositorio
      final orders = await _orderRepository.getOrdersByUserId(userId);
      orders.sort((a, b) => b.id.compareTo(a.id));
      print('✅ [OrderService] Órdenes para UserID $userId obtenidas correctamente.');
      return orders;
    } catch (e) {
      print('❌ [OrderService] Error al obtener las órdenes para UserID $userId: $e');
      rethrow;
    }
  }
}
